...
interface GroupData { // Define locally or import if exported from Sidebar
...
interface DroppableTableSectionProps {
  ...
  onNewGuestSubmit: (event: React.KeyboardEvent<HTMLInputElement>, tableId: string) => void;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2.5">
         ...
         {isUnassigned ? (
           <Badge variant="outline" className="text-xs bg-sidebar-accent/10 text-sidebar-foreground/80 shadow-sm border-sidebar-border/40">
             {occupiedSeats}
           </Badge>
         ) : (
           <Badge variant="outline" className="text-xs bg-sidebar-accent/10 text-sidebar-foreground/80 shadow-sm border-sidebar-border/40 px-2">
      ...
      {/* Add Guest Input */}
      {!isUnassigned && occupiedSeats < totalSeats && (
